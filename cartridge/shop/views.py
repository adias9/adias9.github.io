    ...
    if request.method == "POST":
        if add_product_form.is_valid():
            if to_cart:
               ...
               # bidPrice = add_product_form.cleaned_data["bid_price"] #CHANGED
               # currPrice = add_product_form.variation.price() #CHANGED
               # if bidPrice > currPrice:         #CHANGED
               #     request.cart.unit_price(bidPrice)
               #     info(request, _(currPrice)) """
                request.cart.add_item(add_product_form.variation, quantity)  #CHANGED from quantity to hardcoded 1
                recalculate_cart(request)
                info(request, _("Item added to cart"))
                return redirect("shop_cart")  #I CHANGED THIS!!!! (shop_cart)
            else:
